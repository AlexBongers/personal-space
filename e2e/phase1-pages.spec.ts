...
async function createPage(page: Page, title: string, parentLabel?: string) {
  if (parentLabel) {
    ...
    await page.getByRole("button", { name: `Add page inside ${parentLabel}` }).click();
  } else {
...
async function deletePage(page: Page, label: string) {
  ...
  await expect(page.getByRole("treeitem", { name: new RegExp(label) })).toHaveCount(0);
}
...
test("seeded workspace loads with sidebar tree and icons", async ({ page }) => {
  ...
  await expect(page.getByRole("treeitem", { name: /Home Lab Rebuild/ })).toBeVisible();
  await page.getByRole("button", { name: "Expand Home Lab Rebuild" }).click();
  await expect(page.getByRole("treeitem", { name: /Parts Inventory/ })).toBeVisible();
});
...
test("create a nested page, delete the parent, confirm cascade", async ({ page }) => {
  ...
  await page.getByRole("button", { name: `Page options for ${parent}` }).click();
  await page.getByRole("menuitem", { name: "Delete" }).click();
  await expect(page.getByRole("dialog")).toContainText("everything nested inside it");
  await page.getByRole("dialog").getByRole("button", { name: "Delete" }).click();

  await expect(page.getByRole("treeitem", { name: new RegExp(parent) })).toHaveCount(0);
  await expect(page.getByRole("treeitem", { name: new RegExp(child) })).toHaveCount(0);
...
test("rename from the sidebar options menu", async ({ page }) => {
  ...
  await page.getByRole("button", { name: `Page options for ${before}` }).click();
  ...
  await expect(page.getByRole("treeitem", { name: new RegExp(after) })).toBeVisible();
