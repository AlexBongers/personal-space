...
beforeEach(async () => {
  db = openDb(":memory:");
  app = createApp(db);
  pageId = (await request(app).post("/api/pages").send({ title: "Doc" })).body.id;
});

const addBlock = async (type: string, text: string, index?: number) =>
  (await request(app).post(`/api/pages/${pageId}/blocks`).send({ type, content: { text }, index })).body;

const blockTitles = async () => {
  const page = (await request(app).get(`/api/pages/${pageId}`)).body;
...
describe("blocks API", () => {
  ...
  it("rejects unknown block types and missing pages", async () => {
    const bad = await request(app).post(`/api/pages/${pageId}/blocks`).send({ type: "gif" });
    expect(bad.status).toBe(400);
    const missing = await request(app).post("/api/pages/nope/blocks").send({ type: "paragraph" });
  ...
  it("edits content and converts type", async () => {
    ...
    const patched = await request(app)
      .patch(`/api/blocks/${b.id}`)
      .send({ type: "quote", content: { text: "hello!" } });
    expect(patched.body.type).toBe("quote");
    expect(patched.body.content.text).toBe("hello!");
    expect((await request(app).patch(`/api/blocks/${b.id}`).send({ type: "gif" })).status).toBe(400);
    expect((await request(app).patch("/api/blocks/none").send({})).status).toBe(404);
  });
  ...
  it("deletes a block and compacts positions", async () => {
    ...
    await request(app).delete(`/api/blocks/${a.id}`);
    expect(await blockTitles()).toEqual(["b", "c"]);
    const page = (await request(app).get(`/api/pages/${pageId}`)).body;
    expect(page.blocks.map((b: any) => b.position)).toEqual([0, 1]);
    expect((await request(app).delete("/api/blocks/none")).status).toBe(404);
  });
  ...
  it("rejects partial or foreign id lists on reorder", async () => {
    const a = await addBlock("paragraph", "a");
    await addBlock("paragraph", "b");
    const partial = await request(app).put(`/api/pages/${pageId}/blocks/order`).send({ ids: [a.id] });
