...
import { buildTree } from "../src/routes/pages.js";
...
describe("pages API", () => {
  it("creates a page and returns it in the tree", async () => {
    const created = await request(app).post("/api/pages").send({ title: "Alpha", icon: "🅰️" });
  ...
  it("nests pages under a parent and orders siblings by position", async () => {
    const parent = (await request(app).post("/api/pages").send({ title: "Parent" })).body;
    await request(app).post("/api/pages").send({ title: "First", parentId: parent.id });
    await request(app).post("/api/pages").send({ title: "Second", parentId: parent.id });
  ...
  it("rejects an unknown parent and a bad type", async () => {
    const bad = await request(app).post("/api/pages").send({ title: "X", parentId: "nope" });
    expect(bad.status).toBe(400);
    const badType = await request(app).post("/api/pages").send({ title: "X", type: "row" });
    expect(badType.status).toBe(400);
  });

  it("reads a single page with its blocks array", async () => {
    const page = (await request(app).post("/api/pages").send({ title: "Solo" })).body;
    const res = await request(app).get(`/api/pages/${page.id}`);
    expect(res.status).toBe(200);
    expect(res.body.title).toBe("Solo");
    expect(res.body.blocks).toEqual([]);
    expect((await request(app).get("/api/pages/missing")).status).toBe(404);
  });

  it("renames a page and updates its icon", async () => {
    const page = (await request(app).post("/api/pages").send({ title: "Old" })).body;
    const renamed = await request(app).patch(`/api/pages/${page.id}`).send({ title: "New", icon: "🚀" });
    expect(renamed.body.title).toBe("New");
    expect(renamed.body.icon).toBe("🚀");
    const cleared = await request(app).patch(`/api/pages/${page.id}`).send({ icon: null });
    expect(cleared.body.icon).toBeNull();
    expect((await request(app).patch("/api/pages/missing").send({ title: "x" })).status).toBe(404);
  });

  it("deletes a page and cascades to all descendants", async () => {
    const a = (await request(app).post("/api/pages").send({ title: "A" })).body;
    const b = (await request(app).post("/api/pages").send({ title: "B", parentId: a.id })).body;
    const c = (await request(app).post("/api/pages").send({ title: "C", parentId: b.id })).body;

    const del = await request(app).delete(`/api/pages/${a.id}`);
    expect(del.body.ok).toBe(true);
    for (const id of [a.id, b.id, c.id]) {
      expect((await request(app).get(`/api/pages/${id}`)).status).toBe(404);
    }
    expect((await request(app).delete("/api/pages/missing")).status).toBe(404);
  });
});
