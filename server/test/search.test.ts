...
beforeEach(async () => {
  db = openDb(":memory:");
  app = createApp(db);
  const parent = (await request(app).post("/api/pages").send({ title: "Travel", icon: "✈️" })).body;
  await request(app).post("/api/pages").send({ title: "Japan Trip", parentId: parent.id });
  const dbPage = (await request(app).post("/api/pages").send({ title: "Reading List", type: "database" })).body;
  await request(app).post(`/api/databases/${dbPage.id}/rows`).send({ title: "Japanese Cooking" });
});

describe("search API", () => {
  it("matches page, database, and row titles case-insensitively", async () => {
    const res = await request(app).get("/api/search?q=japan");
  ...
  it("finds databases by title", async () => {
    const res = await request(app).get("/api/search?q=reading");
    expect(res.body[0].type).toBe("database");
  });

  it("ranks prefix matches first", async () => {
    await request(app).post("/api/pages").send({ title: "About Japan" });
    const res = await request(app).get("/api/search?q=japan");
    expect(res.body[0].title).toBe("Japan Trip");
  });

  it("returns an empty list for a blank query and escapes LIKE wildcards", async () => {
    expect((await request(app).get("/api/search?q=")).body).toEqual([]);
    expect((await request(app).get("/api/search?q=%")).body).toEqual([]);
    expect((await request(app).get("/api/search?q=_")).body).toEqual([]);
  });
});
