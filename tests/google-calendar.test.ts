...
test("Google Calendar database exposes editable event fields", () => {
  ...
  assert.ok(database.properties.some((property) => property.id === GOOGLE_CALENDAR_PROPERTY_IDS.recurrence));
});
...
test("Google Calendar payload preserves all-day and timed events", () => {
  const allDay = {
    ...
    values: {
      ...
      [GOOGLE_CALENDAR_PROPERTY_IDS.recurrence]: "RRULE:FREQ=WEEKLY",
    },
    blocks: [],
  };
...
test("Google Calendar event mapping keeps stable identifiers", () => {
  ...
  assert.equal(row.values[GOOGLE_CALENDAR_PROPERTY_IDS.recurrence], "RRULE:FREQ=MONTHLY");
});
