...
describe("api client", () => {
  it("GETs without a body and parses JSON", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/tree", { method: "GET", headers: undefined, body: undefined });
  });
  ...
  it("falls back to a generic message when the error body is not JSON", async () => {
    ...
    await expect(api.deletePage("x")).rejects.toThrow("DELETE /api/pages/x failed (500)");
  });
  ...
  it("hits the expected endpoints for blocks, databases, and search", async () => {
    ...
    expect(urls).toEqual([
      ...
      "/api/blocks/b",
      "/api/blocks/b",
      "/api/pages/p/blocks/order",
      "/api/databases/d",
      "/api/databases/d/properties",
      "/api/properties/pr",
      "/api/properties/pr",
      "/api/properties/pr/options",
      "/api/databases/d/rows",
      "/api/rows/r/values",
      "/api/rows/r",
      "/api/databases/d/views/board",
      "/api/search?q=q%20x",
    ]);
  });
});
